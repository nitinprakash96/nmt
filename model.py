...
class NMT(nn.Module):
    def __init__(self, embed_size, hidden_size, vocab, dropout_rate=0.2):
        ...
        self.encoder = nn.LSTM(
            embed_size, hidden_size, bidirectional=True, bias=True)
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]
        ...
        @returns enc_hiddens (Tensor): Tensor of hidden units with shape (b, src_len, h*2).
    ...
    def decode(self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor,
        ...
        @param enc_masks (Tensor): Tensor of sentence masks (b, src_len).
        @param dec_init_state (tuple(Tensor, Tensor)): Initial state and cell for decoder
        @param target_padded (Tensor): Gold-standard padded target sentences (tgt_len, b).

        @returns combined_outputs (Tensor): combined output tensor  (tgt_len, b,  h).
        """
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        @param enc_hiddens (Tensor): Encoder hidden states Tensor, with shape (b, src_len, h * 2).
        @param enc_hiddens_proj (Tensor): Encoder hidden states Tensor, projected from (h * 2) to h. Shape = (b, src_len, h).
        ...
        x = nn.Softmax(dim=1)
        alpha_t = x(e_t)
        a_x = (alpha_t.size(0), 1, alpha_t.size(1))
        a_t = torch.bmm(alpha_t.view(*a_x), enc_hiddens).squeeze(1)
